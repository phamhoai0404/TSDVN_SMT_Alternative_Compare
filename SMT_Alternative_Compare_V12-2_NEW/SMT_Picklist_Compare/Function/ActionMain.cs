    ...
    public class ActionMain
    {
        public static void ActionCompareETSD(List<Picklist> picklist_1, List<Picklist> picklist_2, List<LKTT_ETSD> listLKTT, ref List<DataOut> listResult)
        {
            foreach (Picklist itemList_1 in picklist_1)
            {
                ...
                {
                    listResult.Add(new DataOut(itemList_1.plItem, itemList_1.plItem));
                }
        ...
        private static void GetDataWhenE(ref List<DataOut> listData, List<LKTT_ETSD> listLKTT, List<Picklist> list2, string itemCurrent)
        {
            ...
            foreach (var item in listWhenE)
            {
                ...
                if (checkExist)
                {
                    listData.Add(new DataOut(item.dataE, item.dataL));
                }
            }
        }
        private static void GetDataWhenL(ref List<DataOut> listData, List<LKTT_ETSD> listLKTT, List<Picklist> list2, string itemCurrent)
        {
            //Thuc hien dang thuoc truong hop nay
            //E1-Y
            //E2-Y
            ...
            foreach (var item in listWhenL)
            {
                ...
                if(tempCheck)
                {
                    listData.Add(new DataOut(itemCurrent, item.dataE));
                } 
                ...
                foreach (var itemRef in listWhenE)
                {
                    ...
                    if (checkExist)
                    {
                        listData.Add(new DataOut(itemCurrent, itemRef.dataL, item.dataE));
                    }
                }
            }
        }
